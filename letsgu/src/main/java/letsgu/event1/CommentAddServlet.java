...
public class CommentAddServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        ArrayList<Comment> list = dao.GetComment();
        
        req.setAttribute("list", list);
        
        if (result > 0) {
            req.getRequestDispatcher("/WEB-INF/views/commentTest.jsp").forward(req, resp);
        } else {
        	req.getRequestDispatcher("/WEB-INF/views/commentTest2.jsp").forward(req, resp);
        }
	}
}
